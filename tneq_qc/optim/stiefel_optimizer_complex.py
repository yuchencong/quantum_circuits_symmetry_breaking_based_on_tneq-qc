...
from .gutils import matrix_norm_one
import random
...
class SGDG(Optimizer):
    ...
    def compute_Y(self, alpha, W, X):
        # I = torch.eye(W.size(0)).cuda()
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                ndim = len(size)
                mid = ndim // 2
                row_dim = int(torch.prod(torch.tensor(size[:mid])))
                col_dim = int(torch.prod(torch.tensor(size[mid:])))
                # import pdb; pdb.set_trace()
                # p_data_permute = torch.permute(p.data, [0, 1, 2, 3])
                p_data_permute = p.data
                # unity,_ = unit(p_data_permute.view(p.size()[0]*p.size()[1],-1))
                unity,_ = unit(p_data_permute.reshape(row_dim, col_dim))
                if stiefel and unity.size()[0] <= unity.size()[1]:
                    ...
                    # p_grad_data_permute = torch.permute(p.grad.data, [0, 1, 2, 3])
                    p_grad_data_permute = p.grad.data
                    # g = p_grad_data_permute.view(p.size()[0]*p.size()[1],-1)
                    g = p_grad_data_permute.reshape(row_dim, col_dim)
                    ...
                    if 'momentum_buffer' not in param_state:
                        ...
                        # if p.is_cuda:
                        #     param_state['momentum_buffer'] = param_state['momentum_buffer']
                        param_state['momentum_buffer'] = torch.zeros(g.t().size(), dtype=g.dtype, device=g.device)
                            
                    V = param_state['momentum_buffer']
                    V = momentum * V - torch.conj(g).t()   
                    MX = torch.mm(V, unity)
                    XMX = torch.mm(unity, MX)
                    XXMX = torch.mm(torch.conj(unity).t(), XMX)
                    W_hat = MX - 0.5 * XXMX
                    W = W_hat - torch.conj(W_hat).t()
                    t = 0.5 * 2 / (matrix_norm_one(W) + episilon)                    
                    alpha = min(t, lr)
                    
                    #p_new = Cayley_loop(unity.t(), W, V, alpha)
                    p_new_t = self.compute_Y(alpha, W, torch.conj(unity).t())
                    p_new = torch.conj(p_new_t).t()

                    V_new = torch.mm(W, torch.conj(unity).t()) # n-by-p
                    # To be modified
                    param_state['momentum_buffer'].copy_(V_new) # save V for the next iteration
#                     check_identity(p_new.t())
                    # p_new_permute = torch.permute(p_new.view(p.size()), [0, 1, 2, 3])
                    p.data.copy_(p_new.reshape(size))
            ...
            for p in group['params']:
                ...
                if stiefel and unity.size()[0] <= unity.size()[1]:
                    ...
                    if 'm_buffer' not in param_state:
                        size=p.size()
                        # param_state['m_buffer'] = torch.zeros([int(np.prod(size[2:])), size[0]*size[1]])
                        ...
                        param_state['m_buffer'] = torch.zeros([int(np.prod(size[2:])), size[0]*size[1]], dtype=g.dtype, device=g.device)
                        param_state['v_buffer'] = torch.zeros([1], dtype=g.dtype, device=g.device)
                    ...
                    # compute the p_new using Cayley Transform
                    p_new = self.compute_Y(alpha, W, unity.t()).t()

                    p_new_permute = torch.permute(p_new.view(p.size()), [0, 1, 2, 3])
                    p.data.copy_(p_new_permute)
                    #p.data.copy_(p_new.view(p.size()))
